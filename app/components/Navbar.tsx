    ...
    <nav className="sticky top-0 z-50 p-4">
      ...
      >
        ...
        <button
          ...
          className="lg:hidden w-11 h-11 grid place-items-center rounded-full bg-[#f4f2ec] hover:bg-[#e6f2ec] transition-colors duration-300 cursor-pointer"
        >
      ...
      {/* Mobile drawer */}
      {open && (
        <div className="lg:hidden border-t border-[#e6f2ec] px-6 py-4 flex flex-col gap-2">
          {links.map((link) => (
            <a
              key={link.label}
              href={link.href}
              onClick={() => setOpen(false)}
              className="text-[#1b2520] text-base font-medium py-2 cursor-pointer"
            >
              {link.label}
            </a>
          ))}
          <a
            href="#"
            className="mt-2 inline-flex items-center justify-center gap-2 bg-[#0b6e4f] text-white text-base font-semibold rounded-full px-6 h-[52px] hover:bg-[#0f996d] transition-colors duration-300 cursor-pointer"
          >
            Get started
