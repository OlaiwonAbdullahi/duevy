...
**Flow:** Standard email/password login. `spaceIds` embedded in the token snapshot the caller's memberships/rep spaces _at login time_ — if a rep's space is provisioned after their last login, call `POST /auth/refresh` (or log in again) to pick it up.
...
{
  "success": true,
  "data": { "avatarUrl": "https://api.duevy.com/uploads/avatars/abc123.jpg" }
}
```
...
```json
{
  ...
  "data": [
    ...
    {
      "id": "sess_2",
      "device": "Duevy iOS App",
      "ip": "102.89.44.2",
      "lastSeenAt": "2026-07-10T18:22:00.000Z",
      "current": false
    }
  ]
}
...
```json
{
  ...
  "data": {
    ...
    "status": "active",
    "membership": "member",
    "joinedAt": "2026-07-15T09:30:00.000Z"
  }
}
```
  ...
  "data": [
    {
      ...
      "description": "Published due \"Dept levy\"",
      "actor": { "id": "usr_7q1m9z2p", "name": "Tunde Okafor" },
...
{
  "success": true,
  "data": { "spaceId": "spc_9f3k2j1", "newLeadId": "usr_3k8p1x9m" }
}
```
    ...
    "students": [
      {
        ...
        "email": "aisha@example.com",
        "status": "paid",
        "paidAt": "2026-07-10T08:30:00.000Z",
        "reference": "DVY-4821-7735"
      },
      {
        "id": "usr_3k8p1x9m",
...
{
  "success": true,
  "data": { "available": 18500000, "pending": 6500000, "lifetime": 42000000 }
}
```
...
```json
{
  ...
  "data": {
    ...
    "accountNumber": "01234•••••",
    "accountName": "ENGINEERING 101 DEPT ASSOCIATION",
    "cooldownUntil": null
  }
}
```
...
**Flow:** "Save" after the rep confirms the name shown by the preview call above. Re-resolves and saves — `accountName` is never client-supplied, only ever server-resolved. Changing an _existing_ account triggers a 24h payout hold and emails every rep of the space as a security notice.
...
```json
{
  ...
  "accountNumber": "0123456789",
  "accountName": "ENGINEERING 101 DEPT ASSOCIATION",
  "cooldownUntil": "2026-07-16T09:30:00.000Z"
}
```
  ...
  "data": [
    {
      ...
      "note": "Venue payment",
      "requestedAt": "2026-07-15T10:15:00.000Z",
      "settledAt": "2026-07-15T10:16:30.000Z",
      "failureReason": null
    }
  ],
  "meta": { "page": 1, "perPage": 20, "total": 1, "totalPages": 1 }
...
{
  "selections": [
    { "categoryId": "cat_1", "nomineeId": "nom_2", "quantity": 3 }
  ],
  "method": "wallet"
}
```
...
{
  "selections": [
    { "categoryId": "cat_1", "nomineeId": "nom_2", "quantity": 1 }
  ],
  "method": "card",
  "cardId": "card_9x2k1"
}
...
```json
{
  ...
  "transactionReference": "DVY-4821-7735",
  ...
  "data": [
    {
      ...
      "detail": "Aisha Bello paid ₦5,000.00 for \"Dept levy\".",
    ...
    "attention": [
      {
        ...
        "tone": "warning",
        "badge": "Verification",
        "title": "3 rep applications awaiting review",
      ...
      },
      {
        ...
        "tone": "warning",
        "badge": "Overdue",
        "title": "6 dues past deadline with unpaid balances",
        "detail": "₦42,000.00 still outstanding across overdue dues.",
        "href": "/admin/spaces",
...
### 14.3 Reps — _userManagement_ (unless noted)
  ...
  "data": [
    {
      "id": "usr_7q1m9z2p",
      "name": "Tunde Okafor",
      "departmentIds": ["spc_9f3k2j1"],
      "status": "active",
      "verification": "verified",
      "heldAmount": 18500000,
      "uncollectedAmount": 17250000,
      "collectionRate": 0.6
    }
  ],
...
**POST** `/v1/admin/reps/{repId}/freeze-payouts` — _payouts_ permission. **Flow:** freezes payouts for every space this rep leads, e.g. during a compliance investigation.
...
**POST** `/v1/admin/reps/{repId}/unfreeze-payouts` — _payouts_ permission. No payload. **Response `204`**.

### 14.4 Spaces — _userManagement_
  ...
  "data": [
    {
      ...
      "memberCount": 84,
      "duesTarget": 43260000,
      "collectedAmount": 25750000,
      "assignedRepIds": ["usr_7q1m9z2p"],
      "isArchived": false,
      "payoutsFrozen": false
    }
  ],
...
{
  "success": true,
  "data": { "userId": "usr_7q1m9z2p", "spaceId": "spc_9f3k2j1", "role": "lead" }
}
```
...
### 14.5 Transactions Oversight — _userManagement_ (refunds need _overrides_)
...
**POST** `/v1/admin/transactions/{txnId}/refund` — _overrides_ permission. **Flow:** "Issue refund" action from a transaction row, e.g. resolving a dispute. Credits the user's wallet.

```json
{
  "amount": 515000,
  "reason": "Duplicate charge confirmed by dispute #dsp_9k1m2x"
}
```
...
```json
{
  ...
  "data": {
    ...
    "type": "refund",
    "title": "Refund",
    "detail": "Duplicate charge confirmed by dispute #dsp_9k1m2x",
    "amount": 515000,
    "method": "Wallet",
    "status": "completed",
    "reference": "DVY-7712-4401",
...
**POST** `/v1/admin/disputes/{id}/resolve` — _overrides_ permission. **Flow:** "Resolve" action after investigation. Notifies the student who opened it.

```json
{
  "resolution": "upheld",
  "note": "Confirmed duplicate charge; refund issued",
  "refundTxnId": "txn_99zz"
}
```
...
**POST** `/v1/admin/polls/{pollId}/close` — _overrides_ permission. **Flow:** force-close any poll regardless of owner, e.g. for policy violations.
...
### 14.8 Referral Integrity — _userManagement_ (resolving flags needs _overrides_)
...
```json
{
  ...
  "data": [
    {
      "id": "flg_1",
      "referrer": "Tunde Okafor",
      "referred": "Fake Student",
      "label": "Suspicious signup pattern",
      "description": "5 referrals from the same device fingerprint within 1 hour",
      "amount": 100000,
      "status": "pending",
...
**POST** `/v1/admin/referrals/flags/{flagId}/resolve` — _overrides_ permission. **Flow:** "Approve" pays the reward out normally, "Void" cancels it without penalty, "Claw back" reverses an already-paid reward.
...
### 14.10 Reports — _userManagement_
...
```json
{
  ...
  "from": "2026-01-01",
  "to": "2026-06-01",
  "spaceId": "optional — scopes space_collection to one space"
}
```
...
interface User {
  ...
  spaces: Array<{
    id: string;
    name: string;
    membership: "member" | "rep";
    joinCode?: string;
  }>; // joinCode present for rep spaces
}
